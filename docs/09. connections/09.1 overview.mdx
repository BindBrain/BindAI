...
BindAI also supports named provider connections for project-level AI provider credentials.

These provider connections are separate from the external-service `Connection` classes described above. They provide a secure way for a project to select a stored provider credential without placing the credential directly in `bindai.toml` or source code.

A project provider connection consists of:
...
The CLI can create and manage these connections:
