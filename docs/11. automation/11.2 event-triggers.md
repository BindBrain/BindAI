...
Background automation workers are a separate higher-level capability.
...
The broader automation package now provides separate abstractions for automation execution state and run history:
...
The broader automation package additionally provides foundational execution-state and run-history abstractions, but those capabilities are not automatically performed by `EventTrigger`.

Background workers, advanced event routing, retry policies, and other higher-level execution capabilities remain separate concerns.
...
The broader automation package now provides separate abstractions for automation definitions, execution state, and run history. Those higher-level capabilities can be composed with event-triggered execution without making persistence or execution management part of the `EventTrigger` abstraction itself.
